...
# Helper script to set up cron jobs for optrack using the simplified architecture
...
echo "===== Opportunity Tracker Cron Setup (Simplified) ====="
echo ""
echo "This script will set up automated grant scanning that runs on your main branch."
echo "Make sure you're on the branch you want to run automated updates from!"
echo ""
...
if grep -q "optrack.*run_optrack_update.sh" "$TEMP_CRONTAB"; then
  echo ""
  echo "⚠️  An OpTrack cron job already exists:"
  grep "optrack.*run_optrack_update.sh" "$TEMP_CRONTAB"
  echo ""
  ...
  grep -v "optrack.*run_optrack_update.sh" "$TEMP_CRONTAB" > "${TEMP_CRONTAB}.new" || true
  mv "${TEMP_CRONTAB}.new" "$TEMP_CRONTAB"
fi

# Add the new cron job
echo "# optrack - Opportunity Tracker (simplified)" >> "$TEMP_CRONTAB"
